WITH RptData AS
(
SELECT [AG].[Roll Year] AS [ROLLYEAR], 
       ...
       [PC].[Property Class] AS [PROPERTY CLASS], 
...
(
    ...
    GROUP BY [dimProperty_SK]
     ...
     LEFT JOIN [edw].[dimResidential]
     ON [PR].[dimProperty_SK] = [edw].[dimResidential].[dimProperty_SK]
     LEFT JOIN [edw].[dimPropertyClass] AS [PC]
     ON [FACT].[dimPropertyClass_SK] = [PC].[dimPropertyClass_SK]
WHERE [AG].[Roll Year] = 2018
)
SELECT * FROM RptData ORDER BY [JUR],[NEIGH],[ROLL NUM]
...
--WHERE([AG].[Area Code] = '22')
--     --AND ([AG].[Neighbourhood Code] IN('701041', '718505'))
--     --AND ([AG].[Neighbourhood Code] = '701041' OR [AG].[Neighbourhood Code] = '718505')
--     --AND ([MC].[Manual Class Code] != 'C424' AND [MC].[Manual Class Code] != 'D424')
--     --AND ([AU].[Actual Use Code] != '287' AND [AU].[Actual Use Code] != '040')
--     --AND (NOT([MC].[Manual Class Code] IN('C424', 'D424')))
--	 --AND (NOT([AU].[Actual Use Code] IN('287', '040')))
--     AND ([AG].[Roll Year] = 2018);