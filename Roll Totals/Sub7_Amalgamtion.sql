...
SET @p_AM = '006';
SELECT [FA].[Roll Year], 
       [JR].[Amalgamation Code], 
       [JR].[Amalgamation], 
	   [JR].[Amalgamation Desc],
...
FROM [edw].[FactAllAssessedAmounts] AS [FA]
     ...
     INNER JOIN [edw].[dimJurisdiction] AS [JR] ON [JR].dimJurisdiction_SK = [AG].[dimJurisdiction_SK]
     INNER JOIN [edw].[FactPropertyClassOccurrenceCount] AS [OC]
     ON [FA].[dimFolio_SK] = [OC].[dimFolio_SK]
WHERE [FA].[Roll Year] = @p_RY
      AND [FA].[Cycle Number] = @p_CN
      AND [JR].[Amalgamation Code] IN(@p_AM)
