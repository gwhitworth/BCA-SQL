...
BEGIN
	...
	SET @addr1 = LTRIM(RTRIM(ISNULL(@addr1,'')) + dbo.FNC_APPEND_DATA(@p_Street_Number, ' ') +
	...
	IF ((LEN(@addr1) + 4 + LEN(@p_address_floor)) > @p_line_length)
		SET @overflow = LTRIM(RTRIM(@overflow +  ' ' + dbo.FNC_APPEND_DATA((dbo.FNC_PREPEND_DATA(@p_address_floor, 'FLR ')), ' ')));
	ELSE
		SET @addr1 = LTRIM(RTRIM(ISNULL(@addr1,'') + ' ' + dbo.FNC_APPEND_DATA((dbo.FNC_PREPEND_DATA(@p_address_floor, 'FLR ')), ' ')));

	SET @addr2 =	LTRIM(RTRIM(ISNULL(@addr2,'') + dbo.FNC_APPEND_DATA((dbo.FNC_PREPEND_DATA(@p_address_site, 'SITE ')), ' ')
					+ dbo.FNC_APPEND_DATA((dbo.FNC_PREPEND_DATA(@p_address_comp, 'COMP ')), ' ')));

	SET @addr3 =	LTRIM(RTRIM(ISNULL(@addr3,'') + dbo.FNC_APPEND_DATA(@p_address_mod, ' ')
						+ dbo.FNC_APPEND_DATA(@p_address_mod_value, ' ')
						+ dbo.FNC_APPEND_DATA(@p_address_dim, ' ')
						+ dbo.FNC_APPEND_DATA(@p_address_dim_value, ' ')));

	SET @addr4 =	LTRIM(RTRIM(ISNULL(@addr4,'') + dbo.FNC_APPEND_DATA(@p_city, ' ')
						+ dbo.FNC_APPEND_DATA(dbo.FNC_SET_PROVINCE(@p_province_state), '     ')
						+ dbo.FNC_APPEND_DATA(@p_postal_zip, ' ')));

	SET @addr5 =	LTRIM(RTRIM(ISNULL(@addr5,'') + dbo.FNC_APPEND_DATA(dbo.FNC_SET_COUNTRY(@p_country), ' ')));
    ...
    IF @p_line_number IS NULL OR @p_line_number = 0  
		SET @rtnValue = UPPER(LTRIM(RTRIM(dbo.FNC_CHECK_LEN(@address, @p_line_length))))
    ELSE --return the line of addr according to the line number
      SET @rtnValue = UPPER(LTRIM(RTRIM(dbo.FNC_GET_ADDRESS_LINE(@address, @p_line_length, @p_line_number))))
