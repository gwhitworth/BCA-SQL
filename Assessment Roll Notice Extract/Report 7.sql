...
CONCAT([TYPE_SEQNUM],IIF(LEN(ROW_NUMBER() OVER (ORDER BY (SELECT 'BCA00020')))>1,'000','0000'),ROW_NUMBER() OVER (ORDER BY (SELECT 'BCA00020'))), 
...
REPLACE(NEWID(), '-', ''), [PREV_VALUE_SET], [EPIN Or Related Pin Number], [PCT_CHANGE_SET], [AREA_NAME], [SD_NAME], [NEIGH_NAME], [INSERT_1_YN], [INSERT_2_YN], [INSERT_3_YN], [INSERT_4_YN] --, CHAR(13) + CHAR(10)
...
--CONCAT([AOL].[Line6], SPACE(59-LEN(ISNULL([AOL].[Line6], '')))) AS [Email Address], 
           ...
           SPACE(20) AS [EPIN Or Related Pin Number], 
           CONCAT(
